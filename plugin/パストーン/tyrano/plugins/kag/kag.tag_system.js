...
tyrano.plugin.kag.tag.return = {
    start: function() {
        ...
        if (pm.caller && pm.caller.storage && "make.ks" == pm.caller.storage && 1 == this.kag.tmp.loading_make_ref) {
...
tyrano.plugin.kag.tag.loadcss = {
    ...
    start: function(pm) {
        var file = pm.file,
            style = '<link rel="stylesheet" href="' + file + "?" + Math.floor(1e7 * Math.random()) + '">';
