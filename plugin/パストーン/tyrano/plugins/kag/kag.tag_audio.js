tyrano.plugin.kag.tag.playbgm = {
  ...
  tyrano.plugin.kag.tag.stopbgm = {
    ...
    start: function (pm) {
      ...
      $.getBrowser();
      for (key in target_map) key == pm.buf && function () {
        var _key = key,
          _audio_obj = null;
        if ("bgm" === pm.target) {
        ...
        "true" == pm.fadeout ? _audio_obj.fade(_audio_obj.volume(), 0, parseInt(pm.time)) : _audio_obj.stop()
      }();
  ...
  tyrano.plugin.kag.tag.playse = {
    ...
    start: function (pm) {
      this.kag.layer.hideEventLayer();
      "true" == pm.clear && this.kag.ftag.startTag("stopbgm", {
        target: "se",
        stop: "true"
      });
      this.kag.ftag.startTag("playbgm", pm)
  ...
  tyrano.plugin.kag.tag.speak_on = {
    ...
    start: function (pm) {
      "speechSynthesis" in window ? this.kag.stat.play_speak = !0 : console.error("*error:この環境は[speak_on]の読み上げ機能に対応していません");
      this.kag.ftag.nextOrder()
    }
  };
